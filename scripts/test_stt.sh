...
if .venv-stt/bin/python -c 'import torch,sys; sys.exit(0 if torch.backends.mps.is_available() else 1)'; then
  ...
  if /usr/bin/time -l .venv-stt/bin/python src/stt_test.py --device mps --dtype float16 --audio output/stt_ru_16k.wav --output output/stt_ru.txt 2>&1 | tee -a logs/stt.log; then
    exit 0
  fi
...
/usr/bin/time -l .venv-stt/bin/python src/stt_test.py --device cpu --dtype float32 --audio output/stt_ru_16k.wav --output output/stt_ru.txt 2>&1 | tee -a logs/stt.log
