...
A second, harder blocker is the interpreter: `sglang-omni` declares `Requires-Python >=3.10,<3.13`, and Colab now runs Python 3.13, so `pip install` refuses before downloading anything. The notebook checks this up front and names it. `TTS_PYTHON` defaults to `"3.12"`, which makes `uv` fetch a standalone interpreter and build a fully independent environment (its own torch — tens of minutes and several gigabytes), because without it the stage cannot run on current Colab at all; set it to `None` to skip instead. The full install transcript is written to `metrics/tts_install.log` on Drive and its tail is embedded in the stage metrics, so a failure is diagnosable after the session ends.
